...
const ResumeUpload = () => {
  ...
  const [parsedDataFile, setParsedDataFile] = useState(null);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setParsedDataFile(res.data.parsedDataFile); // Store the file path!
      setPreviewURL("");
      setDeployedURL("");
    } catch (err) {
  ...
  const generatePortfolio = async () => {
    if (!parsedDataFile) {
      alert("No parsed resume file found. Please upload your resume first.");
      return;
    }
    setLoading(true);
    const token = localStorage.getItem("token");
    try {
      ...
      setPreviewURL("http://localhost:5000/preview?" + Date.now());
    } catch (err) {
  ...
  const applyCustomization = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/customize-portfolio",
  ...
  const deployPortfolio = async () => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/deploy-portfolio",
        {},
        {
          headers: {
            "Authorization": `Bearer ${token}`,
